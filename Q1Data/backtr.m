function [t] = backtr(t,xc,grad_x,F,gamma,alpha,rhok)
...
elseif (nargin < 7)
    ...
    t = t;
    while (F(xc+t.*grad_x)>F(xc)-gamma*t^2*(norm(grad_x))^2) 
        if (t*norm(grad_x) < rhok)   
            t  = 0;              % <-- failure to search for a value of alpha nonzero
        else
            t = t*alpha;     % <-- reduction of the steplength
        end
    end 
    alpha1 = t;
    F1     = F(xc+alpha1.*grad_x)-(F(xc)-gamma*alpha1^2*(norm(grad_x))^2);
    ...
    alpha2 = -t;
    F2     = F(xc+alpha2.*grad_x)-(F(xc)-gamma*alpha2^2*(norm(grad_x))^2);
